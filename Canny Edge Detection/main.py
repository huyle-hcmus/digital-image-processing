...
# Function to apply Sobel filters to an image
def sobel_filters(img):
    ...
    for i in range(1, len(img)-1): # Height
        for j in range(1, len(img[0]) - 1): # Width
            ...
            gy[i][j] = (img[i - 1][j - 1] + 2*img[i - 1][j] + img[i - 1][j + 1]) - (img[i + 1][j - 1] + 2*img[i + 1][j] + img[i + 1][j + 1])
...
def non_max_suppression(img, D):
    ...
    for i in range(1,M-1):
        for j in range(1,N-1):
                # Check pixel orientation and perform non-maximum suppression
                #angle 0
...
def hysteresis(img, weak, strong):
    for i in range(1, len(img) - 1): 
        for j in range(1, len(img[0]) - 1):
            # Check for weak pixels and perform edge tracking
            if (img[i,j] == weak):
                try:
                    if (
                        (img[i+1, j-1] == strong) 
